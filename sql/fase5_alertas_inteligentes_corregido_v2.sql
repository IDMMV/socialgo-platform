...
-- Se usa un CTE porque PostgreSQL no permite referenciar el alias de la tabla
-- objetivo del UPDATE dentro de esta expresión LATERAL del FROM.
with coordenadas_publicas as (
  select
    a.id as alerta_id,
    c.latitud,
    c.longitud
  from public.alertas a
  join public.alerta_ubicaciones_privadas p
    on p.alerta_id = a.id
  cross join lateral public.mizona_coordenada_publica(
    p.latitud_exacta,
    p.longitud_exacta,
    a.precision_ubicacion,
    a.id::text
  ) c
)
update public.alertas a
set latitud = cp.latitud,
    longitud = cp.longitud
from coordenadas_publicas cp
where cp.alerta_id = a.id;
