...
async function load(){
 ...
 const [{data:p,error:pe},{data:r,error:re}]=await Promise.all([
  supabase.from('perfiles').select('nombre_visible,telefono_contacto').eq('id',user.id).maybeSingle(),
 ...
 setValue('#driverNames',request?.nombres||profile.nombre_visible||user.user_metadata?.full_name||'');setValue('#driverPhone',String(request?.celular||profile.telefono_contacto||'').replace(/\D/g,'').slice(-9));
 ...
 if(request){badge(request.estado);if(request.estado==='aprobado'){show('✅ Tu cuenta de conductor está aprobada. Ya puedes ponerte en línea desde MiZonaRide.','success');submit.textContent='Actualizar documentos';}else if(request.estado==='observado'){show(`La solicitud tiene observaciones: ${request.observacion_admin||'Revisa tus datos y documentos.'}`,'warning');}else if(request.estado==='rechazado'){show(`La solicitud fue rechazada: ${request.observacion_admin||'Puedes corregir y volver a enviarla.'}`,'error');}else show('Tu solicitud está en revisión. Puedes agregar documentos faltantes.');}
...
form.addEventListener('submit',async e=>{e.preventDefault();submit.disabled=true;submit.textContent='Enviando…';try{const args={p_nombres:$('#driverNames').value,p_celular:$('#driverPhone').value.replace(/\D/g,'').slice(-9),p_dni:$('#driverDni').value,p_licencia_numero:$('#driverLicense').value,p_licencia_categoria:$('#driverLicenseCategory').value,p_licencia_vencimiento:$('#driverLicenseExpiry').value||null,p_vehiculo_marca:$('#vehicleBrand').value,p_vehiculo_modelo:$('#vehicleModel').value,p_vehiculo_anio:Number($('#vehicleYear').value)||null,p_vehiculo_color:$('#vehicleColor').value,p_placa:$('#vehiclePlate').value,p_soat_vencimiento:$('#soatExpiry').value||null,p_revision_vencimiento:$('#technicalExpiry').value||null};const {data:id,error}=await supabase.rpc('mizona_enviar_solicitud_conductor',args);if(error)throw error;await uploadDocs(id);show('✅ Solicitud enviada. El administrador revisará tus datos y documentos.','success');await load();}catch(e){show(errText(e),'error');}finally{submit.disabled=false;submit.textContent='Enviar para revisión';}});
load();

$('#driverPhone')?.addEventListener('input',e=>{e.target.value=e.target.value.replace(/\D/g,'').slice(0,9)});
